...
from schrodinger.application.desmond.packages.energygroup import analyze, EnergyGroupBase

def parse_args(argv):
    parser = argparse.ArgumentParser(description="""Calculate energy properties from MD simulation, example:
    ...
    parser.add_argument('-e', action='append', help='''Expression of type "NAME:ASL1:[ASL2]:TYPE(s)" where:
...
class GroupEnergy(EnergyGroupBase):
    ...
    def __init__(self, cms_model, group1, type, group2=None):
        ...
        """
        self.key = (group1, group2)  # DUCK: this is used to cache the results, since all energy types are actually calculated
        ...
        self.kwargs = {'options': self.DEFAULT_OPTIONS, 'groups': groups}  # DUCK: required by EnergyFacade
...
def main(*argv):
    ...
    slicer = slice(*[int(v) if v else None for v in args.s.split(':')]) if args.s else slice(None, None)
    trj = trj[slicer]
