...
def bkey2str(key):
    return f'{" ".join([str(i) for i in key[0]])} - {" ".join([str(i) for i in key[1]])}'
...
def main():
    ...
    parser.add_argument("out", help="output base filename (descriptive extensions are added automatically)")
    ...
    parser.add_argument("-p", "--plot", help="plot bond nr over time", action="store_true")
    parser.add_argument("-rkey", help="comma-separated indexes to slice the key that is used to identify unique reisudes:\
        key = [atom.chain, atom.pdbres, atom.resnum, atom.element, atom.index] \
    ...
    for a in args.b.split(','):
        if a == 'hb':
            analyzers.append(analysis.HydrogenBondFinder(msys, cms, aids1=aids1, aids2=aids2))
            btype.append('HB')
        elif a == 'sb':
            analyzers.append(analysis.SaltBridgeFinder(msys, cms, aids1=aids1, aids2=aids2))
            btype.append('SB')
        elif a == 'pipi':
    ...
    for data, btype in zip(out, btype):
        InterOut = InteractionOutput(data, cms)
        ...
        write_allbonds(fh_allbonds, allbonds, times)
        fh_allbonds.close()
