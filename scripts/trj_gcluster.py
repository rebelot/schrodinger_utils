...
def gromos(rmsd, threshold, nmax):
    def gromos_inner(the_rmsd, the_threshold, n_max):
        ...
        if n_max > 1 and len(not_members_indices) > 0:
            other_results = gromos_inner(
                the_rmsd[not_members, :][:, not_members], the_threshold, n_max - 1
            )
...
def find_threshold(rmsd, nmax, method="closest"):
    ...
    raw_thresholds = np.linspace(
        rmsd.max() * 0.01,
        rmsd.max() * 0.99,
        endpoint=True,
    )[1:]
    raw_clusters_list = [
        find_clusters(rmsd, threshold, nmax) for threshold in raw_thresholds
    ]
    raw_nonlocality = [
        compute_nonlocality(assigned, unassigned)
        for (assigned, unassigned) in raw_clusters_list
    ]
    first = 0
    last = 0
    ...
    thresholds = np.linspace(
        raw_thresholds[first], raw_thresholds[last], endpoint=True, num=200
    )
    clusters_list = [find_clusters(rmsd, threshold, nmax) for threshold in thresholds]
    nonlocality = [
        compute_nonlocality(assigned, unassigned)
        for (assigned, unassigned) in clusters_list
    ]
...
def write_log(outname, result, trajectory):
    ...
    for index, item in enumerate(assigned):
        log.write(
            f"Cluster # {index + 1:2d}, medoid is frame {trajectory[item[0]].orig_index:5d}, "
            f"number of frames {len(item[1])}\n"
        )
    if len(unassigned) > 0:
        log.write(
            f'{len(unassigned)} frames were further than {result["threshold"]}'
...
def write_clusters(outbase, result, the_trj, the_cms, split):
    ...
    for i, (med, mem) in enumerate(clusters):
        ...
        the_cms.fix_filenames(
            f"{outbase}_cluster_{i + 1}-out.cms", f"{outbase}_cluster_{i + 1}_trj"
        )
...
if __name__ == "__main__":
    args = argparse.ArgumentParser(
        description="Perform RMSD-based clustering of a Desmond MD with gromos algorithm."
    )
    args.add_argument("cms", help="Cms input file")
    args.add_argument("output", help="Output base name")
    args.add_argument(
        "-rmsd_asl", help="Define ASL for RMSD calculation", default="a.pt CA"
    )
    args.add_argument("-fit_asl", help="Define ASL for fitting;", default=None)
    args.add_argument(
        "-t", help="RMSD threshold defining clusters (Angstroms)", type=float
    )
    args.add_argument(
        "-n", help="Max number of clusters returned", default=None, type=int
    )
    args.add_argument(
        "-w",
        "--write_matrix",
        help="Store intermediate distance matrix to FILE",
        default=None,
    )
    args.add_argument(
        "-l",
        "--load_matrix",
        help="Load intermediate distance matrix from FILE",
        default=None,
    )
    args.add_argument("-s", "--slice", help="slice trajectory START:END:STEP")
    args.add_argument(
        "-split_trj", help="save cluster frames in trajectories", action="store_true"
    )
    args.add_argument(
        "-m",
        "--method",
        choices=["closest", "largest"],
        help='crietrion to assign frames to medoids. Default "closest" (ucluster)',
        default="closest",
    )
    ...
    if args.load_matrix:
        ...
        rmsd_gids = topo.aids2gids(
            cms, cms.select_atom(args.rmsd_asl), include_pseudoatoms=False
        )
        fit_gids = topo.aids2gids(
            cms, cms.select_atom(args.fit_asl), include_pseudoatoms=False
        )
        rmsd_matrix = analysis.rmsd_matrix(msys, trj, rmsd_gids, fit_gids)
        if args.write_matrix:
            print(f"saving distance matrix to {args.write_matrix}")
            data = {"slicer": slicer, "cms": args.cms, "rmsd": rmsd_matrix}
